...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data } = await admin.from('favorite_sessions').select('items').eq('device_id', deviceId).limit(1);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const admin = createClient(url, serviceRole);
    await admin.from('favorite_sessions').upsert({ device_id: deviceId, items });
