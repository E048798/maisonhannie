...
export async function POST(req: NextRequest) {
  try {
    ...
    const status_history = Array.isArray(body?.status_history) ? body.status_history : []
    ...
    const { data, error } = await admin.from('orders').update({ status, status_history }).eq('id', id).select('*').limit(1)
